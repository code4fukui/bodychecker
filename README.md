...
QRのリンク先を変更する場合は、管理画面の `設定` を押し、`URLプリフィックス` にQRデータの直前までのURLを入力して保存します。設定値はブラウザの `localStorage` に保存され、次回起動時にも使用されます。`初期値に戻す` を押して保存すると、標準の利用者向け画面に戻せます。
